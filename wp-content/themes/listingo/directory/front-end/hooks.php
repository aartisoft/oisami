...
if (!function_exists('listingo_result_avatar')) {
    ...
    $show_category  = '';
        ...
        if (isset($media_type) && $media_type === 'banner') {
            $thumb = apply_filters(
                    'listingo_get_media_filter', listingo_get_user_banner(array('width' => 370, 'height' => 370), $user_id), array('width' => 370, 'height' => 370)
            );
        } else {
            $thumb = apply_filters(
                    'listingo_get_media_filter', listingo_get_user_avatar(array('width' => 370, 'height' => 370), $user_id), array('width' => 370, 'height' => 370)
            );
        }
...
if (!function_exists('listingo_sort_users_by_featured_expiry')) {
	function listingo_sort_users_by_featured_expiry($a, $b){  //The function to order our authors
	  ...
	  if ($a->subscription_featured_expiry == $b->subscription_featured_expiry) {  //This is where the name of our custom meta key is entered, I named mine "order"
		return 0;  
	  } else if (empty( $b->subscription_featured_expiry )) {
		  return 0;  
	  }
		
	  return ( $b->subscription_featured_expiry < $a->subscription_featured_expiry ) ? -1 : 1;  //The actual sorting is done here. Change ">" to "<" to reverse order
	} 
}
