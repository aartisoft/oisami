...
$cliente          = !empty($_GET['cliente'])          ? esc_attr($_GET['cliente'])          : '';
$cliente_atendido = !empty($_GET['cliente_atendido']) ? esc_attr($_GET['cliente_atendido']) : '';
$recomendacao     = !empty($_GET['recomenda'])        ? esc_attr($_GET['recomenda'])        : '';
...
//cliente_atendido Search
if (!empty($cliente_atendido)) {
    $meta_query_args[] = array(
        'key' => 'vm50_clientes_atendidos',
        'value' => $cliente_atendido,
        'compare' => '='
    );
}
...
if (!empty($recomendacao)) {
    $recomendacoes = explode(',', $recomendacao);
    $meta_query_args[] = array(
        'key' => 'vm50_recomendacao_medica',
        'value' => $recomendacoes,
        'compare' => 'IN'
    );
}
