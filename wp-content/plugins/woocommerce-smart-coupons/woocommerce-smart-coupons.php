	...
	if ( ! class_exists( 'WC_Smart_Coupons' ) ) {
		...
		class WC_Smart_Coupons {
			...
			private function __construct() {
				...
				add_option( 'sc_gift_certificate_shop_loop_button_text', __( 'Select options', WC_SC_TEXT_DOMAIN ) );
			...
			public function includes() {
				...
				include_once 'includes/class-wc-sc-privacy.php';
			...
			public function get_expiration_format( $expiry_date ) {
				...
				if ( $expiry_days < 1 ) {
					...
					$expires_string = __( 'Expires in ', WC_SC_TEXT_DOMAIN ) . $expiry_days . _n( ' day', ' days', $expiry_days, WC_SC_TEXT_DOMAIN );
			...
			public function sa_email_coupon( $coupon_title, $discount_type, $order_id = '', $gift_certificate_receiver_name = '', $message_from_sender = '', $gift_certificate_sender_name = '', $gift_certificate_sender_email = '', $is_gift = '' ) {
				...
				foreach ( $coupon_title as $email => $coupon ) {
					...
					switch ( $discount_type ) {
						...
						case 'fixed_cart':
							$email_heading  = sprintf( __( 'You have received a coupon worth %1$s (for entire purchase) ', WC_SC_TEXT_DOMAIN ), wc_price( $amount ) );
							break;

						case 'fixed_product':
							if ( ! empty( $_product_ids ) || ! empty( $_excluded_product_ids ) || ! empty( $_product_categories ) || ! empty( $_excluded_product_categories ) ) {
								$_discount_for_text = __( 'for some products', WC_SC_TEXT_DOMAIN );
							} else {
								$_discount_for_text = __( 'for all products', WC_SC_TEXT_DOMAIN );
							}
							$email_heading  = sprintf( __( 'You have received a coupon worth %1$s (%2$s) ', WC_SC_TEXT_DOMAIN ), wc_price( $amount ), $_discount_for_text );
							break;
						...
						case 'percent_product':
							...
							$email_heading  = sprintf( __( 'You have received a coupon worth %1$s%% (%2$s) ', WC_SC_TEXT_DOMAIN ), $amount, $_discount_for_text );
							break;
						...
						case 'percent':
							if ( ! empty( $_product_ids ) || ! empty( $_excluded_product_ids ) || ! empty( $_product_categories ) || ! empty( $_excluded_product_categories ) ) {
								...
								$_discount_for_text = __( 'for entire purchase', WC_SC_TEXT_DOMAIN );
							}
							$email_heading  = sprintf( __( 'You have received a coupon worth %1$s%% (%2$s) ', WC_SC_TEXT_DOMAIN ), $amount, $_discount_for_text );
							break;
			...
			public function register_plugin_styles() {
				...
				$is_valid_admin_page = ( ! empty( $_GET['page'] ) && $_GET['page'] == 'wc-smart-coupons' ) ? true : false;
			...
			public function sc_handle_store_credit_application() {
				...
				if ( $this->is_wc_gte_30() && wc_tax_enabled() && $apply_before_tax == 'yes' ) {
					include_once 'includes/class-wc-sc-apply-before-tax.php';
				} else {
			...
			public function order_calculate_discount_amount( $and_taxes, $order ) {
				...
				if ( $order instanceof WC_Order && ! empty( $_POST['action'] ) && ( in_array( $_POST['action'] , $order_actions ) || ( ! empty( $_POST['post_type'] ) && $_POST['post_type'] == 'shop_order' && $_POST['action'] == 'editpost' ) ) ) {
						...
						if ( sizeof( $coupon_product_ids ) > 0 || sizeof( $coupon_product_categories ) > 0 ) {
							...
							foreach ( $order_items as $order_item_id => $order_item ) {
								...
								if ( sizeof( $coupon_product_ids ) > 0 ) {
									...
									if ( ! $continue && in_array( $order_item['product_id'], $coupon_product_ids ) || in_array( $order_item['variation_id'], $coupon_product_ids ) ) {
			...
			public function smart_coupons_after_calculate_totals( $cart = null ) {
				...
				if ( ! empty( $cart_total ) ) {
					...
					$total = $this->smart_coupons_discounted_totals( $cart_total, $cart );
			...
			public function manage_smart_coupon_credit_used( $coupon = '', $discount = 0, $cart_contains_subscription = false, $calculation_type = '' ) {
				if( is_object( $coupon ) && $coupon instanceof WC_Coupon ) {
					...
					if ( empty( $smart_coupon_credit_used[ $coupon_code ] ) || ( $cart_contains_subscription && ( $calculation_type == 'combined_total' || $calculation_type == 'sign_up_fee_total' ) ) ) {
			...
			public function generate_smart_coupon_action( $email, $amount, $order_id = '', $coupon = '', $discount_type = 'smart_coupon', $gift_certificate_receiver_name = '', $message_from_sender = '', $gift_certificate_sender_name = '', $gift_certificate_sender_email = '' ) {
				...
				foreach ( $emails as $email_id => $qty ) {
					...
					/**
					 * new_coupon_id 		newly generated coupon post id
					 * ref_coupon 			Reference coupon. This is the coupon from which meta will be copied to newly created coupon
					 */
					do_action( 'wc_sc_new_coupon_generated', array( 'new_coupon_id' => $smart_coupon_id, 'ref_coupon' => $coupon ) );
			...
			public function woocommerce_admin_coupon_search( $wp ) {
				...
				if ( 'Email:' == substr( $wp->query_vars['s'], 0, 6 ) ) {
					...
					$query = $wpdb->prepare( 'SELECT post_id FROM ' . $wpdb->postmeta . ' WHERE meta_key=%s AND meta_value LIKE %s; ', 'customer_email', '%' . $wpdb->esc_like( $email ) . '%' );
			...
			public function woocommerce_admin_coupon_search_label( $query ) {
				...
				if ( $email ) {
					...
					return sprintf( __( "[%s with email: %s]", WC_SC_TEXT_DOMAIN ), $post_type->labels->singular_name, $email );
				}
			...
			public function generate_coupons_code( $post, $get, $post_ids, $coupon_postmeta_headers = array() ) {
				...
				if ( ! empty( $get ) && isset( $get['export_coupons'] ) ) {
					...
					$query_to_fetch_data = $wpdb->prepare( "SELECT p.ID,
															  p.post_title,
															  p.post_excerpt,
															  p.post_status,
															  p.post_parent,
					...
					foreach ( $results as $result ) {
						...
						foreach ( $coupon_meta_key as $index => $key ) {
							if ( $key == 'product_ids' || $key == 'exclude_product_ids' ) {
								...
								$data[ $id ][ $key ] = (is_serialized( $coupon_meta_value[ $index ] )) ? implode( ',', maybe_unserialize( stripslashes( $coupon_meta_value[ $index ] ) ) ) : $coupon_meta_value[ $index ];
							}
						}
			...
			public function export_coupon( $post, $get, $post_ids ) {
				...
				$coupon_postmeta_headers = apply_filters( 'wc_smart_coupons_export_headers',
					array(
						...
						'usage_count'                => __( 'Usage Count',WC_SC_TEXT_DOMAIN ),
						'_used_by'                   => __( 'Used By',WC_SC_TEXT_DOMAIN ),
				) );
			...
			public function smart_coupon_styles_and_scripts() {
				...
				if ( ! empty( $pagenow ) ) {
					...
					if ( $pagenow == 'admin.php' && ! empty( $_GET['page'] ) && $_GET['page'] == 'wc-smart-coupons' ) {
						$show_css_for_smart_coupon_tab = true;
					}
