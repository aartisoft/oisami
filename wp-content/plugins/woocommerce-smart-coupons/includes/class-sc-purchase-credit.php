...
if ( ! class_exists( 'WC_SC_Purchase_Credit' ) ) {
	...
	class WC_SC_Purchase_Credit {
		...
		public function woocommerce_cart_item_price_html( $product_price, $cart_item, $cart_item_key ) {
			...
			$gift_certificate = WC()->session->get( 'credit_called' );
			...
			foreach ( WC()->cart->cart_contents as $product ) {
				...
				if ( $coupon_titles ) {
					...
					foreach ( $coupon_titles as $coupon_title ) {
						...
						$smart_coupon_gift_certificate_form_page_text  = ( ! empty( $smart_coupon_gift_certificate_form_page_text ) ) ? $smart_coupon_gift_certificate_form_page_text : __( 'Send Coupons to...', WC_SC_TEXT_DOMAIN );
		...
		public function add_text_field_for_email( $coupon = '', $product = '' ) {
			...
			for ( $i = 1; $i <= $product['quantity']; $i++ ) {
				...
				// TODO: Free Gift Coupon: Due to coupon amount as zero, following condition is not shoing multiple form to send coupon to different people
