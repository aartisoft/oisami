	...
	class WC_SC_WCS_Compatibility {
		...
		public function sc_wcs_modify_renewal_order_meta( $renewal_order = null, $subscription = null ) {
			...
			if ( ! empty( $old_order_item_ids ) ) {
				...
				$query_to_fetch_product_ids = $wpdb->prepare( "SELECT woim.order_item_id,
																(CASE
				...
				foreach ( $coupon_used_in_original_order as $coupon_code ) {
					...
					if ( ! empty( $discount_type ) && $discount_type == 'smart_coupon' && ! empty( $coupon_amount ) && ! in_array( $coupon_code, $coupon_used_in_renewal_order, true ) ) {
						...
						if ( $discount > 0 ) {
							...
							$used_by = ( is_object( $renewal_order ) && is_callable( array( $renewal_order, 'get_user_id' ) ) ) ? $renewal_order->get_user_id() : 0;
							if ( ! $used_by ) {
		...
		public function smart_coupons_settings( $settings = array() ) {
			...
			$wc_subscriptions_options = array(
												array(
													'name'              => __( 'Recurring Subscriptions', WC_SC_TEXT_DOMAIN ),
													'desc'              => __( 'Use store credit applied in first subscription order for subsequent renewals until credit reaches zero', WC_SC_TEXT_DOMAIN ),
												...
												array(
													'desc'              => __( 'Renewal orders should not generate coupons even when they include a product that issues coupons', WC_SC_TEXT_DOMAIN ),
			...
			array_splice( $settings, ( count( $settings ) - 12 ), 0, $wc_subscriptions_options ) ;
		...
		public function sc_wcs_styles_and_scripts() {
			...
			<script type="text/javascript">
				jQuery(function(){
					jQuery('#woocommerce_smart_coupon_apply_before_tax').on('change', function(){
						var current_color = jQuery('#pay_from_smart_coupon_of_original_order').css('background-color');
						jQuery('#pay_from_smart_coupon_of_original_order').parent().effect('highlight', { color: '#ff8866' }, 1500);
						if ( jQuery(this).is(':checked') ) {
							jQuery('#pay_from_smart_coupon_of_original_order').prop('checked', false);
						} else {
							jQuery('#pay_from_smart_coupon_of_original_order').prop('checked', true);
						}
					});
				});
			</script>
		...
		public function sc_wcs_settings() {
			...
			if ( $is_apply_before_tax == $is_pay_from_store_credit ) {
				if ( $is_apply_before_tax == 'yes' ) {
					update_option( 'pay_from_smart_coupon_of_original_order', 'no' );
				} else {
					update_option( 'pay_from_smart_coupon_of_original_order', 'yes' );
				}
			}
		}
