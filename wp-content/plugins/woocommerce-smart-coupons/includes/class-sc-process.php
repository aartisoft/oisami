...
if ( ! class_exists( 'WC_SC_Coupon_Process' ) ) {
	...
	class WC_SC_Coupon_Process {
		...
		public function delete_smart_coupons_contribution() {
			...
			if ( ! empty( $_POST['action'] ) && $_POST['action'] == 'woocommerce_remove_order_coupon' && ! empty( $_POST['order_id' ] ) &&  ! empty( $_POST['coupon' ] ) ) {
				$smart_coupons_contribution = get_post_meta( $_POST['order_id' ], 'smart_coupons_contribution', true );

				if ( isset( $smart_coupons_contribution[ $_POST['coupon'] ] ) ) {
					unset( $smart_coupons_contribution[ $_POST['coupon'] ] );
					if ( empty( $smart_coupons_contribution ) ) {
						delete_post_meta( $_POST['order_id' ], 'smart_coupons_contribution' );
					} else {
						update_post_meta( $_POST['order_id' ], 'smart_coupons_contribution', $smart_coupons_contribution  );
					}
				}
				...
				if( $order->has_status( array( 'on-hold' ) ) ) {
					$this->sa_restore_smart_coupon_amount( $_POST['order_id'] );
				}
			}
		}
		...
		public function update_smart_coupon_balance( $order_id ) {
			...
			if ( ! empty( $_POST['post_type'] ) && $_POST['post_type'] == 'shop_order' && ( $_POST['action'] == 'editpost' || $_POST['action'] == 'woocommerce_mark_order_status' ) ) {
				return;
			}
			...
			if ( ! empty( $coupon_titles ) ) {
				...
				foreach ( $coupon_titles as $coupon_title ) {
					...
					if ( ( $auto_generation_of_code == 'yes' || $discount_type == 'smart_coupon' ) && $operation == 'add' ) {
						...
						$is_auto_generate = $amount > 0 || $is_free_shipping == 'yes';

						$is_auto_generate = apply_filters( 'wc_sc_is_auto_generate', $is_auto_generate, array( 'coupon_id' => $coupon_id, 'auto_generate' => $auto_generation_of_code, 'coupon_obj' => $coupon ) );
			...
			if ( is_array( $sc_called_credit_details ) && count( $sc_called_credit_details ) > 0 && $operation == 'add' ) {
				...
				foreach ( $order_items as $item_id => $item ) {
					...
					if ( $this->is_wc_gte_30() ) {
						...
						$product_id = ( in_array( $product_type, array( 'variable', 'variable-subscription', 'variation', 'subscription_variation' ) ) ) ? ( ( is_object( $product ) && is_callable( array( $product, 'get_parent_id' ) ) ) ? $product->get_parent_id() : 0 ) : ( ( is_object( $product ) && is_callable( array( $product, 'get_id' ) ) ) ? $product->get_id() : 0 );
					} else {
