	...
	class WC_SC_Apply_Before_Tax {
		...
		public function order_calculate_discount_amount_before_tax( $and_taxes, $order ) {
			...
			if ( $order instanceof WC_Order && ! empty( $_POST['action'] ) && ( in_array( $_POST['action'] , $order_actions )  || ( ! empty( $_POST['post_type'] ) && $_POST['post_type'] == 'shop_order' && $_POST['action'] == 'editpost' ) ) ) {
				...
				if ( empty( $order_items ) && empty( $coupons ) ) return;
						...
						if ( is_array( $smart_coupons_contribution ) && count( $smart_coupons_contribution ) > 0 && array_key_exists( $coupon_code, $smart_coupons_contribution ) ) {
							...
							if ( count( $coupon_product_ids ) > 0 || count( $coupon_category_ids ) > 0 ) {
								foreach( $order_items as $order_item_id => $order_item ) {
									...
									if (  count( $coupon_product_ids ) > 0 && count( $coupon_category_ids ) > 0  ) {
                       					if ( ( in_array( $order_item['product_id'], $coupon_product_ids ) || in_array( $order_item['variation_id'], $coupon_product_ids ) ) && count( array_intersect( $product_category_ids , $coupon_category_ids ) ) > 0 ) {
						...
						if ( $discount_type === 'smart_coupon' ) {
							...
							if ( count( $coupon_product_ids ) > 0 || count( $coupon_category_ids ) > 0 ) {
								foreach ( WC()->cart->cart_contents as $cart_item_key => $cart_item ) {
									...
									if (  count( $coupon_product_ids ) > 0 && count( $coupon_category_ids ) > 0  ) {
                       					if ( ( in_array( $cart_item['product_id'], $coupon_product_ids ) || in_array( $cart_item['variation_id'], $coupon_product_ids ) ) && count( array_intersect( $product_category_ids , $coupon_category_ids ) ) > 0 ) {
