		...
		public function parse_coupon( $item ) {
			...
			if ( isset( $postmeta['expiry_date'] ) ) {
				...
				if ( ! empty( $postmeta['expiry_date'] ) && empty( $timestamp_expiry_date ) ) {
					$this->log( 'error', sprintf(__( 'Incorrect format for expiry date of coupon "%s". Entered date is %s. Expected date format: YYYY-MM-DD', WC_Smart_Coupons::$text_domain ), $coupon['post_title'], $postmeta['expiry_date'] ) );
				}
				$postmeta['expiry_date'] = ( ! empty( $timestamp_expiry_date )) ? date( 'Y-m-d', $timestamp_expiry_date ) : '';
			}
