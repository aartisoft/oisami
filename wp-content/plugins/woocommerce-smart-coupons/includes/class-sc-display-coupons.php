...
if ( ! class_exists( 'WC_SC_Display_Coupons' ) ) {
	...
	class WC_SC_Display_Coupons {
		...
		public function show_available_coupons_before_checkout_form() {
			...
			$smart_coupon_cart_page_text = ( ! empty( $smart_coupon_cart_page_text ) ) ? $smart_coupon_cart_page_text : __( 'Available Coupons (click on a coupon to use it)', WC_SC_TEXT_DOMAIN );
		...
		public function show_attached_gift_certificates() {
			...
			if ( $coupon_titles && count( $coupon_titles ) > 0 && ! empty( $price ) ) {
				...
				$smart_coupons_product_page_text = ( ! empty( $smart_coupons_product_page_text ) ) ? $smart_coupons_product_page_text : __( 'You will get following coupon(s) when you buy this item:', WC_SC_TEXT_DOMAIN );
					...
					switch ( $discount_type ) {
						...
						case 'fixed_product':
							if ( ! empty( $product_ids ) || ! empty( $excluded_product_ids ) || ! empty( $product_categories ) || ! empty( $excluded_product_categories ) ) {
								$discount_on_text = __( 'some products', WC_SC_TEXT_DOMAIN );
							} else {
								$discount_on_text = __( 'all products', WC_SC_TEXT_DOMAIN );
							}
							$amount = wc_price( $coupon_amount ) . __( ' discount on ', WC_SC_TEXT_DOMAIN ) . $discount_on_text;
							break;
						...
						case 'percent_product':
							...
							$amount = $coupon_amount . '%' . __( ' discount on ', WC_SC_TEXT_DOMAIN ) . $discount_on_text;
							break;
						...
						case 'percent':
							if ( ! empty( $product_ids ) || ! empty( $excluded_product_ids ) || ! empty( $product_categories ) || ! empty( $excluded_product_categories ) ) {
								...
								$discount_on_text = __( 'your entire purchase', WC_SC_TEXT_DOMAIN );
							}
							$amount = $coupon_amount . '%' . __( ' discount on ', WC_SC_TEXT_DOMAIN ) . $discount_on_text;
							break;
		...
		public function show_smart_coupon_balance() {
			...
			$smart_coupons_myaccount_page_text = ( ! empty( $smart_coupon_myaccount_page_text ) ) ? $smart_coupon_myaccount_page_text: __( 'Available Coupons & Store Credits', WC_SC_TEXT_DOMAIN );
		...
		public function get_coupon_html( $coupon_data = array() ) {
			...
			$html .= '<div class="coupon-container apply_coupons_credits red medium" name="' . $coupon_data['coupon_code'] . '" style="cursor: pointer">
						<div class="coupon-content red dashed small" name="' . $coupon_data['coupon_code'] . '">
							<div class="discount-info" >';

			if ( ! empty( $coupon_data['coupon_amount'] ) ) {
				$html .= $coupon_data['coupon_amount'] . ' ' . $coupon_data['coupon_type'];
				if ( $coupon_data['is_free_shipping'] == 'yes' ) {
					$html .= __( ' &amp; ', WC_SC_TEXT_DOMAIN );
				}
			}

			if ( $coupon_data['is_free_shipping'] == 'yes' ) {
				$html .= __( 'Free Shipping', WC_SC_TEXT_DOMAIN );
			}
			$html .= '</div>';
			...
			if ( ! empty( $coupon_data['expiry_date'] ) ) {
				...
				$html .= '<div class="coupon-expire">' . __( 'Never Expires', WC_SC_TEXT_DOMAIN ) . '</div>';
			}
		...
		public function sc_endpoint_content() {
			...
			$available_coupons_heading = ( ! empty( $available_coupons_heading ) ) ? $available_coupons_heading : __( 'Available Coupons & Store Credits', WC_SC_TEXT_DOMAIN );
			?>
			...
			foreach ( $coupons as $code ) {
				...
				if ( in_array( $code->post_title, $coupons_applied ) ) { 
					continue;
				}
			...
			<div id='sc_coupons_list'>
				<h4><?php echo __( 'Store Credits', WC_SC_TEXT_DOMAIN ); ?></h4>
				<div id="all_coupon_container">
					...
					if ( ! empty( $smart_coupons_block ) ) {
						echo $smart_coupons_block;
					}

					?>
				</div>
				<?php
				if ( ! empty( $coupon_block_data['smart_coupons']['total'] ) && $coupon_block_data['smart_coupons']['total'] != 0 ) {
					?>
					<div class="wc_sc_total_available_store_credit"><?php echo sprintf( __( 'Total Credit Amount: %s', WC_SC_TEXT_DOMAIN ), wc_price( $coupon_block_data['smart_coupons']['total'] ) ); ?></div>
					<?php
				}
				?>
			<br>
			...
			<div id='coupons_list'>
				<h4><?php echo __( 'Discount Coupons', WC_SC_TEXT_DOMAIN ); ?></h4>
				<div id="all_coupon_container">
					<?php
			...
			if ( $is_show_invalid_coupons == 'yes' ) {
				?>
				<br>
				<div id='invalid_coupons_list'>
					<h2><?php echo __( 'Invalid / Used Coupons', WC_SC_TEXT_DOMAIN ); ?></h2>
					<div id="all_coupon_container">
						<?php
		...
		public function sc_get_available_coupons_list( $coupons = array() ) {
			...
			$global_coupons = $wpdb->get_results( $query );
			...
			if ( is_user_logged_in() ) {
				...
				if ( ! empty( $current_user->user_email ) && ! empty( $current_user->ID ) ) {
					$query_customer_credit = $wpdb->prepare( "SELECT option_name FROM {$wpdb->prefix}options
																WHERE option_name LIKE %s
																ORDER BY option_id DESC",
															$wpdb->esc_like( 'sc_display_custom_credit_' . $current_user->ID . '_' ) . '%'
															);
					$count_option_current_user = $wpdb->get_col( $query_customer_credit );
					...
					$wpdb->query( $wpdb->prepare( 'SET SESSION group_concat_max_len=%d', 999999 ) );

					$wpdb->query( 
									$wpdb->prepare( "INSERT INTO {$wpdb->prefix}options (option_name, option_value, autoload)
														SELECT %s,
					...
					$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}options WHERE option_name = %s", $option_nm ) );
				}
			}
		...
		public function get_generated_coupon_data( $order_ids = '', $user_ids = '', $html = false, $header = false, $layout = 'box' ) {
			...
			if ( ! empty( $user_ids ) ) {
				...
				$user_order_ids_query = $wpdb->prepare( "SELECT DISTINCT postmeta.post_id FROM {$wpdb->prefix}postmeta AS postmeta
				...
				if ( count( $user_ids ) == 1 ) {
					...
					$user_order_ids_query .= $wpdb->prepare( " IN ( " . implode( ',', $placeholders ) . " )", $user_ids );
				}
			...
			if ( $html ) {
				...
				if ( $found_coupon ) {
					...
					if ( $header ) {
						echo '<h4>' . __( 'Coupon Received', WC_SC_TEXT_DOMAIN ) . '</h4>';
					}
