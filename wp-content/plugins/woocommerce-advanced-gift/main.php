...
/*
	...
	*  Plugin URI: http://proword.net/Plugins/Advanced_Gift
...
class woocommerce_advanced_gift
{
    ...
    {
        //$this->includes();
        add_action('init', array($this, 'woo_advanced_gift_js_css'));
        ...
        //Shortcode Ui
        add_filter('init', array($this, 'wc_gift_shortcodes_add_scripts'));
    ...
    public function includes()
    {
        ...
        */
        ...
        $setting = get_option("pw_gift_options");
        if (!is_admin() && $setting['hide_popup_in_mobile'] == 'true' && wp_is_mobile()) {
            return;
        }
        if (!is_admin() && !isset($_COOKIE['pw_woo_popup_gift_cookie']) && $setting['show_popup'] == "true") {
            require('core/popup.php');
            setcookie("pw_woo_popup_gift_cookie", 'ok', time() + 3600, COOKIEPATH, COOKIE_DOMAIN);
    ...
    public function action_links($links)
    {
        return array_merge(array(
            '<a href="' . admin_url('admin.php?page=rule_gift') . '">' . __('Settings', 'woocommerce-brands') . '</a>',
    ...
    private function show_level_tab()
    {
        ...
        if (@$_GET['pw_action_type'] == "edit")
            $arr = "Edit Rules";
        $tabs = array(
            array('name' => "<i class='fa fa-list-ul' ></i>Rules List", 'url' => "gift_rules"),
            array('name' => "<i class='fa fa-cog' ></i>" . $arr, 'url' => "add_rule"),
            array('name' => "<i class='fa fa-cog' ></i>Settings", 'url' => "setting"),
            array('name' => "<i class='fa fa-cog' ></i>Reports", 'url' => "report"),
        );
        ...
        $tabs=array('Pricing Rules','Setting');
        echo '<h2>';
        ...
        if (@$_GET['tab'] == "report") {
            ...
            <div class="pw-form-content pw-report-tab">
	            <div class="pw-report-subtab">
		            <a href="' . admin_url('admin.php?page=rule_gift&tab=' . $a['url'] . '&subtab=main&pw_action_type=list') . '" class="' . (!isset($_GET['subtab']) || $_GET['subtab'] == "main" ? "active-subtab" : '') . '">' . __('Dashboard', 'pw_wc_advanced_gift') . '</a>';

            echo '<a href="' . admin_url('admin.php?page=rule_gift&tab=' . $a['url'] . '&subtab=rules&pw_action_type=list') . '" class="' . (isset($_GET['subtab']) && $_GET['subtab'] == "rules" ? "active-subtab" : '') . '">' . __('Rules', 'pw_wc_advanced_gift') . '</a>';

            echo '<a href="' . admin_url('admin.php?page=rule_gift&tab=' . $a['url'] . '&subtab=customer&pw_action_type=list') . '" class="' . (isset($_GET['subtab']) && $_GET['subtab'] == "customer" ? "active-subtab" : '') . '">' . __('Customer', 'pw_wc_advanced_gift') . '</a>';

            echo '<a href="' . admin_url('admin.php?page=rule_gift&tab=' . $a['url'] . '&subtab=guest&pw_action_type=list') . '" class="' . (isset($_GET['subtab']) && $_GET['subtab'] == "guest" ? "active-subtab" : '') . '">' . __('Guest', 'pw_wc_advanced_gift') . '</a>  
                </div>';
...
//        }
    ...
    /*	public function add_menu()
        {
            add_menu_page( 'Test Plugin Page', 'Test Plugin', 'manage_options', 'my_custom_menu_page', array( $this, 'my_custom_menu_page' ));
        }
    */
    ...
    public function woo_advanced_gift_js_css()
    {
        if (is_admin()) {
            ...
            //Slider
            //wp_enqueue_style('pw-gift-slider-style', plugin_dir_url_wc_advanced_gift . 'css/frontend/bx-slider/jquery.bxslider.css');
            ...
            //Slider
            //wp_enqueue_script('pw-gift-slider-jquery', plugin_dir_url_wc_advanced_gift . 'js/frontend/bx-slider/jquery.bxslider.js');
    ...
    function wc_gift_shortcodes_add_scripts()
    {
        if (!is_admin()) {
            ...
            wp_enqueue_style('font_awesome', plugin_dir_url_wc_advanced_gift . 'css/font-awesome/font-awesome.min.css');
